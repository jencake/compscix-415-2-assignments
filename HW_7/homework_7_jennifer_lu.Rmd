...
```{r, warning=FALSE}
sim1a_lm_list <- rep(NA, 6)
sim1a_tidy_list <- rep(NA, 6)
sim1a_glance_list <- rep(NA, 6)
for(i in 1:6) {
  sim1a <- tibble(
    x = rep(1:10, each = 3),
  ...
  sim1a_lm_list[i] <- sim1a_lm
  sim1a_tidy_list[i] <- tidy(sim1a_lm)
  sim1a_glance_list[i] <- glance(sim1a_lm)
}
sim1a_lm_list
sim1a_tidy_list
sim1a_glance_list
